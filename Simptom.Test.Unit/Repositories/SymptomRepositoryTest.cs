...
{
	[TestClass]
	public class SymptomRepositoryTest : ISymptomRepositoryTest
	{
		...
		public override void TestInitialize()
		{
			...
			this.repository = new SymptomRepository(this.connection.Object, this.modelFactory.Object);
		}
		...
		public override void AddRow(ISymptom symptom)
		{
			DataRow row = this.dataSet.Tables[0].NewRow();
			
			row["ID"] = symptom.Key.ID;
			
			row["CategoryID"] = symptom.Category.Key.ID;
			row["CategoryName"] = symptom.Category.Name;
			row["Name"] = symptom.Name;
